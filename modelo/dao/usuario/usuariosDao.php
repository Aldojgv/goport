...
class usuarioDao {
    ...
    function  registrarUsuarioDao($apaterno, $amaterno, $nombre, $usuario, $clave, $tipo, $status){
      ...
      if($query->num_rows==0)
      {
        $st = "INSERT INTO tbl_usuario(apaterno, amaterno, nombre, usuario, clave, tipo, status, fRegistro) 
    ...
    function saveDataUsuarioDao($id, $apaterno, $amaterno, $nombre, $usuario, $clave, $tipo, $status) {
      $st = "UPDATE tbl_usuario SET apaterno='$apaterno', amaterno='$amaterno', nombre='$nombre', usuario='$usuario', clave='$clave', tipo='$tipo', status='$status' WHERE idUsuario = '$id'";
      $query = $this->con->query($st); 
    ...
    function eliminarUsuarioDao($usuario) {
      $st = "DELETE FROM tbl_usuario WHERE usuario='$usuario'";
      $query = $this->con->query($st); 
    ...
    function actualizarUsuarioDao($usuario) {
      $cad = "";
      $st = "SELECT * FROM tbl_usuario WHERE usuario = '$usuario'";
